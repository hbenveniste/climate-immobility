...
@defcomp scenconverter_ineq begin
    regions = Index()
    ...
    inequality  = Parameter(index=[time,regions])
    ...
    scenineqgrowth  = Variable(index=[time,regions])
    ineq0           = Variable(index=[regions])
    ...
    function run_timestep(p, v, d, t)
        ...
        for r in d.regions
            ...
            if t.t < 1050
                v.scenineqgrowth[t, r] = (p.inequality[t+1, r] / p.inequality[t, r] - 1.) * 100.
            else
                v.scenineqgrowth[t, r] = v.scenineqgrowth[t - 1, r]
            end
        end
    end
end
