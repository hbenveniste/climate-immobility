...
for s in ssps
    ...
    ) |> save(joinpath(@__DIR__, "../results/migflow_ineq/", string("leave_cata_quint_ccshare_",s,"_v5_update.png")))
end
...
for s in ssps
    for d in ["damageprop","damageindep","damageinvprop"]
        @vlplot(width=800, height=600) + @vlplot(mark={:geoshape, stroke = :lightgray}, 
            ...
            projection={type=:naturalEarth1}, title = {text=string(s,", catastrophic damages"),fontSize=24}, 
        ...
        ) |> save(joinpath(@__DIR__, "../results/world_maps_ineq/", string("leave_cata_q1_ccshare_", s, "_", d, "_v5_update.png")))
    end
end
